...
1. The GPS receiver automatically powers down.
2. The bus is temporarily hidden from the live public feed.
3. The UI transforms into the `RETURN_READY` state.

Start Return Trip

Clicking Start Return Trip:
1. Reconstructs the GTFS geometry dynamically in reverse.
2. Flips the origin and destination stops.
3. Reignites the GPS broadcaster.
