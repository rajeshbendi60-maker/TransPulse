...
1. `SECRET_KEY`: A cryptographically secure random string.
2. `FLASK_ENV`: Set this strictly to `production`.
3. `MAIL_USERNAME` / `MAIL_PASSWORD`: SMTP credentials for the system's automated email notifications.
4. `DATABASE_URL` is automatically provisioned and injected by Render via the blueprint.
...
1. The Start Command defined in `render.yaml` is `gunicorn app:app`.
2. Static files (CSS, JS, Images, Leaflet assets) are served directly by the application and cached via the integrated ServiceWorker.
